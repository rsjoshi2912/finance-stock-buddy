...
def test_pages_read_real_api_data(client):
    ...
    assert data['today_summary']['pending']==10
    ...
    checks={check['name'] for check in client.get('/api/health').json()['checks']}
    assert not checks.intersection({'AI helpers','F&O'})
    assert {'Morning cutoff','Price checks','Scheduler','Latest prices','News feeds'}<=checks
