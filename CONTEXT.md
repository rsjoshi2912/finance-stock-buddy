...
## 18. Flexible selection and concise interface approved (2026-09-09)

The owner explicitly removed the hard five-Buy/five-Sell requirement and approved using the available candidates. This supersedes the quota in sections 2, 4.4 and 8 and earlier handoffs. New daily lists select up to ten eligible candidates overall by the existing confidence-edge × expected-move score, with a deterministic symbol tie-breaker and no direction minimum. Fewer than ten and one-sided days are valid. The ten-position ceiling preserves the existing maximum ₹10,000 stock allocation; empty eligible sets still produce a status note. All eligible candidates and both baselines remain recorded; selection metadata identifies `ranked_available_v1`. The price probability rule itself is unchanged.

The owner also requested a professional, data-focused interface rather than repeated investment warnings. Prioritize calls, results, current prices, news and operational status. Keep one clear generated-data/paper label, and keep actionable missing-data, stale-quote, expired-setup and failed-job states visible. Put methodology, source detail and calculator assumptions behind optional details. Remove duplicated risk lectures and inactive AI placeholders; do not remove actual data checks or imply trained confidence, option execution or proven performance.

Apply the selection change prospectively. Do not rerank saved calls, recreate the skipped September 9 morning later in the day, replay its already-delivered notification, or relax information cutoffs and verified-session checks. The standard P&L, costs, baseline comparison and index-option gates are unchanged.
