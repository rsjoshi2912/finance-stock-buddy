...
Daily selection approved on 2026-09-09 uses up to ten highest-ranked eligible candidates overall, with no minimum per direction. Fewer calls, all Buy or all Sell are valid. No eligible candidates still produces a visible no-calls status. The eligibility, publication/receipt cutoff, ₹1,000 per selected stock and morning window are unchanged. Saved calls, historical skipped days and notification claims are never reranked, backfilled or replayed by this change.
...
At that September 7 verification, the Oracle VM was still timing out during SSH checks. Later Oracle/Pages deployment is recorded in [MEMORY.md](MEMORY.md); those later checkpoints supersede this old connectivity observation. Current local tests are not a new deployment. Operating instructions remain in [DEPLOYMENT.md](DEPLOYMENT.md).
...
Use **Fetch latest** in Today's Market overview to request fresh price snapshots and news immediately, including outside the scheduled collection window. The backend runs the request in the background, so the page stays usable. Progress is saved and survives a browser reload. Detailed source results are expandable, with partial/failed results expanded by default. Price/news panels reload when the request finishes. Market timestamps and stale/delayed status remain visible; repeated source explanations live in optional details.
...
Historical September 9 cash coverage: all 20 stocks had the required pre-cutoff prices, but the five-day rule yielded 3 Up and 17 Down candidates. The then-active five-Buy/five-Sell requirement skipped the entire daily batch and its morning message. That recorded skip remains unchanged. The later approved available-candidate policy applies to new batches, not a retrospective repair of this day. Expanding the verified universe remains a coverage improvement, not a prerequisite for publishing a smaller or one-sided list. Index checks stay separate.

The September 9 notification repair records candidate shortfalls separately from forecasts and surfaces the saved reason in Today and System health. Live `/api/today` and undated brief previews use the actual IST date; explicit historical dates return the original calls. Under the new policy, any nonempty list receives the regular brief, while an empty day receives a no-calls note during the normal message window. The prior confirmed September 9 status delivery is preserved; importing data later cannot create or resend a missed morning batch.
