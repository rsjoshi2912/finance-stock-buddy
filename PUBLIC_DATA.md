...
The September 9 notification repair records candidate shortfalls separately from forecasts, surfaces the reason in Today and System health, and sends a no-calls status note during the normal message window. Live `/api/today` and undated brief previews now use the actual IST date; explicit historical dates continue to return the original calls. The forecast still needs five supported calls per direction, and importing data later cannot create a missed morning batch.
